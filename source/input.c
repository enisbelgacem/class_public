...
/**NonLocal: auxiliary fields initial conditions*/
  pba->Minfl=1e-8;/*default value in GeV to have deltaN<10*/
  class_read_double("Minfl",pba->Minfl);
  double deltaN=64.-log(1e16/pba->Minfl);
if (deltaN<10.){
  pba->U_ini_nlde = 0.;
  ...
  pba->V_prime_ini_nlde = 0.;}
    ...
    const double hundred=1e5/_c_;/*100 km/s/Mpc converted in 1/Mpc*/
    const double MPl=1.2209e+19;/*Planck mass measured in GeV*/
    const double lPl=5.18525e-58;/*Planck length measured in Mpc*/
    double Hinfl=sqrt(8.*_PI_/3.)/lPl*(pba->Minfl/MPl)*(pba->Minfl/MPl);
    const double omegar=4.18343e-5;
    double xend=log(omegar)/4.-0.5*log(Hinfl/hundred);
    double xstart=log(ppr->a_ini_over_a_today_default);
    double a1=1.;
    double c1=-a1*(2+sqrt(13.)+sqrt(21.))/sqrt(13.)/(sqrt(13.)-1);
    double c2=c1*(1+sqrt(13.))/2;
    double d1=-a1*(2-sqrt(13.)+sqrt(21.))/sqrt(13.)/(sqrt(13.)+1);
    double d2=d1*(1-sqrt(13.))/2;
...
/**NonLocal: read model flag: 0. -> LCDM, 1. -> RR, 2. -> RT, 3. -> D4*/
  class_read_double("model",pba->model);
  if (pba->model != 0.)
      pba->has_nlde = _TRUE_;

/**delta_0_gw*/ 
 class_read_double("delta_0_gw",pba->delta_0_gw);

/**ngw*/
 class_read_double("ngw",pba->ngw);
