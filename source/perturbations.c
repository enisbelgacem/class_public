...
/**NonLocal* modified einstein equations 0i and ij offd*/
      ...
      ppw->pvecmetric[ppw->index_mt_phi_prime] = -a_prime_over_a * ppw->pvecmetric[ppw->index_mt_psi] + 1.5 * (a2/k2) * ppw->rho_plus_p_theta;
          ...
          /* equation for psi */
	  ppw->pvecmetric[ppw->index_mt_psi] = ((1-3.*(pba->gnl)*(ppw->pvecback[pba->index_bg_V_nlde])-(pba->gnl)*pow((ppw->pvecback[pba->index_bg_V_prime_nlde])/a/(pba->H0),2))*y[ppw->pv->index_pt_phi]+3.*(pba->gnl)*y[ppw->pv->index_pt_deltaV_nlde]*(1.-(ppw->pvecback[pba->index_bg_U_nlde])/a2)- 4.5 * (a2/k2) * (ppw->rho_plus_p_shear)-(pba->gnl)*(ppw->pvecback[pba->index_bg_V_prime_nlde])*y[ppw->pv->index_pt_deltaV_prime_nlde]/a2/(pba->H0)/(pba->H0))/(1.-3.*(pba->gnl)*(ppw->pvecback[pba->index_bg_V_nlde])+(pba->gnl)*pow((ppw->pvecback[pba->index_bg_V_prime_nlde])/a/(pba->H0),2));
	  /*equation for phi'. Here the variable deltaU is not the variation of U*/
	  ppw->pvecmetric[ppw->index_mt_phi_prime] = -a_prime_over_a * (ppw->pvecmetric[ppw->index_mt_psi]) + (1.5 * (a2/k2) * (ppw->rho_plus_p_theta)-3.*(pba->gnl)*((y[ppw->pv->index_pt_deltaV_prime_nlde]-(ppw->pvecback[pba->index_bg_V_prime_nlde])*(ppw->pvecmetric[ppw->index_mt_psi]))*(1.-(ppw->pvecback[pba->index_bg_U_nlde])/a2/3.)+((ppw->pvecback[pba->index_bg_U_prime_nlde])/a2/2.+k2*(ppw->pvecback[pba->index_bg_V_prime_nlde])/a2/(pba->H0)/(pba->H0)/2.-a_prime_over_a)*(y[ppw->pv->index_pt_deltaV_nlde])+(ppw->pvecback[pba->index_bg_V_prime_nlde])*(y[ppw->pv->index_pt_deltaU_nlde])/a2/6.+(ppw->pvecback[pba->index_bg_V_prime_nlde])*(ppw->pvecback[pba->index_bg_U_nlde])*y[ppw->pv->index_pt_phi]/a2/3.)/2.)/(1.-3.*(pba->gnl)*(ppw->pvecback[pba->index_bg_V_nlde]));
      }
...
/**NonLocal*/
      ...
      if (pba->model == 3.){
          ppw->pvecmetric[ppw->index_mt_gw_prime_prime] = (-(2.*a_prime_over_a*(1-3.*(pba->gnl)*(ppw->pvecback[pba->index_bg_V_nlde]))+(pba->gnl)*(ppw->pvecback[pba->index_bg_V_prime_nlde])*(4.*(ppw->pvecback[pba->index_bg_U_nlde])/a/a-3.))*y[ppw->pv->index_pt_gwdot]-k2*y[ppw->pv->index_pt_gw]*(1.-3.*(pba->gnl)*(ppw->pvecback[pba->index_bg_V_nlde])-(pba->gnl)*pow((ppw->pvecback[pba->index_bg_V_prime_nlde])/a/(pba->H0),2))+(ppw->gw_source))/(1.-3.*(pba->gnl)*(ppw->pvecback[pba->index_bg_V_nlde])+ 2.*(pba->gnl)*pow((ppw->pvecback[pba->index_bg_V_prime_nlde])/a/(pba->H0),2));
      }
  }
  return _SUCCESS_;
...
/**NonLocal*/
      ...
      else if (pba->model == 3.){/*deltaU is not the perturbation of U*/
	psiprime=(-(1.-3.*gnl*V- 2.*gnl*pow(Vprime/a/(pba->H0),2))*metric_continuity/3+3.*gnl*Vprime*(metric_euler/k2-y[pv->index_pt_phi])-9.*a_prime_over_a*a2*(ppw->rho_plus_p_shear)/k2-4.5*a2*rhopluspsigmaprime/k2+3.*gnl*y[pv->index_pt_deltaV_prime_nlde]*(1.+2.*a_prime_over_a*Vprime/a2/(pba->H0)/(pba->H0)/3.-4.*U/a2/3.)-gnl*y[pv->index_pt_deltaU_nlde]*Vprime/a2+3.*gnl*y[pv->index_pt_deltaV_nlde]*(2.*a_prime_over_a*U-Uprime)/a2+2.*gnl*Vprime*(a_prime_over_a*Vprime/(pba->H0)/(pba->H0)-2.*U)*(metric_euler/k2+y[pv->index_pt_phi])/a2)/(1.-3.*gnl*V+2.*gnl*pow(Vprime/a/(pba->H0),2));                                                
        phiprimeprime=-(metric_euler*(3.*a_prime_over_a*a_prime_over_a+2.*a*pvecback[pba->index_bg_H_prime])/k2+a_prime_over_a*(-2.*metric_continuity/3.+psiprime)-(metric_euler-k2*y[pv->index_pt_phi])/3.-3.*(-gnl*((6.*a2-U)*y[pv->index_pt_deltaU_nlde]*(pba->H0)*(pba->H0)-2.*metric_euler*(Vprime*Uprime+6.*a2*a_prime_over_a*Vprime+2.*pow(Vprime/(pba->H0),2)*k2/3.)/k2+(Uprime+6.*a2*a_prime_over_a+5.*k2*Vprime/3./(pba->H0)/(pba->H0))*y[pv->index_pt_deltaV_prime_nlde]+(18.*pow(a*a_prime_over_a,2)+12.*a*a*a*pvecback[pba->index_bg_H_prime]-k2*U+4.*k2*a2)*y[pv->index_pt_deltaV_nlde]+Vprime*y[pv->index_pt_deltaU_prime_nlde]+2.*a2*Vprime*metric_continuity+2.*(Uprime*Vprime-pow(U*(pba->H0),2)+6.*U*pow(a*(pba->H0),2)-2.*k2*pow(Vprime/(pba->H0),2)/3.)*y[pv->index_pt_phi])/a2/6.+a2*(ppw->delta_p))/(1.-3.*gnl*V)/2.);                              
                    dy[pv->index_pt_deltaU_nlde]=y[pv->index_pt_deltaU_prime_nlde];                                              
                    dy[pv->index_pt_deltaU_prime_nlde]=-2.*k2*y[pv->index_pt_deltaU_nlde]+(-metric_continuity/3.+psiprime)*(Uprime-k2*Vprime/3./(pba->H0)/(pba->H0)-6.*a2*a_prime_over_a)-6*phiprimeprime*a2+4.*a_prime_over_a*a2*metric_continuity+2.*metric_euler*(a2-2.*U/3.)-4.*(3.*a2*(a*pvecback[pba->index_bg_H_prime]+2*a_prime_over_a*a_prime_over_a)+k2*(a2+U/3.))*y[pv->index_pt_phi]-pow(k2/(pba->H0),2)*y[pv->index_pt_deltaV_nlde];                                                          dy[pv->index_pt_deltaV_nlde]=y[pv->index_pt_deltaV_prime_nlde]; 
                    dy[pv->index_pt_deltaV_prime_nlde]=(pba->H0)*(pba->H0)*y[pv->index_pt_deltaU_nlde]+Vprime*(psiprime-metric_continuity/3.)+2.*U*(y[pv->index_pt_phi]+metric_euler/k2)*(pba->H0)*(pba->H0);
      }
