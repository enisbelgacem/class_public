...
cdef class Class:
    ...
    def luminosity_distance_gw(self, z):
        """
        luminosity_distance(z)
        """
        cdef double tau=0.0
        ...
        if background_at_tau(&self.ba, tau, self.ba.long_info,
                self.ba.inter_normal, &last_index, pvecback)==_FAILURE_:
            raise CosmoSevereError(self.ba.error_message)
        lum_distance_gw = pvecback[self.ba.index_bg_lum_distance_gw]
        free(pvecback)
        return lum_distance_gw
    ...
    def get_current_derived_parameters(self, names):
        ...
        for name in names:
            ...
            elif name == 'delta_0_gw':
                value = self.ba.delta_0_gw
            else:
